...
def get_categories():
    ...
    category_wrapper = soup.find_all("nav")

    for nav in category_wrapper:
        if nav.get("aria-labelledby") == "categoryLinksNav":
            lis = nav.find_all("li")
            ...
            for li in lis:
                ...
                categories[li.text] = "https://www.baby-walz.de" + li.find("a").get("href")
...
def collect(url, categoryname):
    ...
    try:
        ...
        with open("outbaby.html", "w", encoding="UTF-8") as file:
